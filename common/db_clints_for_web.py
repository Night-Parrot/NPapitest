        ...
        for sql_dict in self.sql_list:
            for db_clint_name in sql_dict:
                try:
                    ...
                    # 我太难了
                    cf_dict = re.findall(('(\w+)'), db_clint_name) #拆分步骤，分解完后是['db_name', 'dict_name']
                    if len(cf_dict) == 2:
                        # sql = re_fun_for_web.re_fun_sql(sql_dict[db_clint_name], self.keyv)
                        sql = str(base_tool.replace_for_web(sql_dict[db_clint_name], self.keyv)).replace('\\', '')
                        ...
                        r = json.dumps(r, ensure_ascii=False)
                        r = json.loads(r)
                        logger.info('JSON后的查询结果：' + str(r))
                        r = base_tool.lower2upper(sql, r)
                        logger.info('更新后查询结果：' + str(r) + '类型：' + str(type(r)))
                        sqlinfo = {**sqlinfo, **r[0]}
                    else:
                        return False
