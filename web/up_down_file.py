...
def uploadfile(): #上传文件
    ...
    sql_xm_bh = "SELECT c_bh FROM db_apitesting.t_at_xmxx WHERE c_url = '%s';" % (xmdz)
    try:
        xmbh = all_dbc.pg_select_operator(sql_xm_bh)
        if len(xmbh) == 0:
    ...
    if Suffix in app.config['ALLOWED_EXTENSIONS']:
        ...
        sql_addyl = "INSERT INTO db_apitesting.t_at_ylxx(c_bh, c_bh_xm, c_ylmc, dt_scsj, dt_zxzxsj, n_zxcs, c_bclj) "\
                    "VALUES ('%s', '%s', '%s', now(), NULL, 0, '%s');" % (ylbh, xmbh, file_name, newpath_all)
        try:
